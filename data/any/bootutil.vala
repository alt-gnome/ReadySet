...
int main (string[] args) {
    var wm = Path.build_filename (Config.LIBEXECDIR, Config.NAME + "-wm");
    var app = Path.build_filename (Config.LIBEXECDIR, Config.NAME);

    if (!FileUtils.test (wm, FileTest.IS_EXECUTABLE)) {
        warning ("WM wrapper '%s' does not exist or is not executable", wm);
        return 0;
    }
    ...
    try {
        message ("Starting '%s' via '%s'", app, wm);
        var process = new Subprocess.newv (
            { "greetd", "--config=" + Path.build_filename (Config.PKG_DATADIR, "any/greetd.toml") },
            NONE
        );

...
public class DateAndTime.TimezoneListRow : Gtk.Box {
    ...
    public TimezoneListItem item {
        ...
        set {
            ...
            var city = Markup.escape_text (value.city);
            var country = Markup.escape_text (value.country);

            subtitle = @"$(city) <b>$(country)</b>";
...
public class DateAndTime.TimezoneListItem : Object {
    ...
    public TimezoneListItem (string entry) {
        ...
        region = dgettext ("gnome-control-center-2.0-timezones", identifier).replace ("_", " ");
    ...
    public static TimezoneListItem? from_identifier (string identifier) {
        var file = File.new_build_filename (Config.DATADIR, "zoneinfo", "zone1970.tab");
        ...
        try {
            ...
            foreach (var entry in entries) {
                ...
                var id = parts[1];
                if (id == identifier) {
                    return new TimezoneListItem (entry);
                }
            }
        } catch (Error e) {
            warning ("Failed to find timezone: %s", e.message);
        }

...
static void
ui_to_setting (CEPageWireguard *self, GError **error)
{
    ...
    GList *peers = NULL;
    guint num_peers = 0;
    for (widget = gtk_widget_get_first_child (GTK_WIDGET (self->peers_box)); widget != NULL;
    ...
    g_list_free (peers);
    g_object_set (self->setting_connection, NM_SETTING_CONNECTION_INTERFACE_NAME,
                  gtk_editable_get_text (GTK_EDITABLE (self->entry_ifname)), NM_SETTING_CONNECTION_ID,
                  gtk_editable_get_text (GTK_EDITABLE (self->entry_conname)), NULL);

    g_object_set (self->setting_wireguard, NM_SETTING_WIREGUARD_PRIVATE_KEY,
                  gtk_editable_get_text (GTK_EDITABLE (self->entry_private_key)), NM_SETTING_WIREGUARD_FWMARK,
                  (guint32) gtk_spin_button_get_value_as_int (self->spin_fwmark), NM_SETTING_WIREGUARD_MTU,
                  (guint32) gtk_spin_button_get_value_as_int (self->spin_mtu), NM_SETTING_WIREGUARD_LISTEN_PORT,
                  (guint32) gtk_spin_button_get_value_as_int (self->spin_listen_port), NULL);
    secret_flags = nma_utils_menu_to_secret_flags (GTK_WIDGET (self->entry_private_key));
    nm_setting_set_secret_flags ((NMSetting *) self->setting_wireguard, NM_SETTING_WIREGUARD_PRIVATE_KEY, secret_flags,
                                 NULL);
    nma_utils_update_password_storage (GTK_WIDGET (self->entry_private_key), secret_flags,
                                       (NMSetting *) self->setting_wireguard, NM_SETTING_WIREGUARD_PRIVATE_KEY);
}

static gboolean
ce_page_wireguard_validate (CEPage *page, NMConnection *connection, GError **error)
{
...
{
    gtk_entry_set_visibility (entry_private_key, !gtk_entry_get_visibility (entry_private_key));
}
...
handle_peer_changed_cb (GtkButton *apply_button, WireguardPeer *wg_peer)
{
    ...
    if (!nm_wireguard_peer_set_endpoint (nm_wg_peer, endpoint && endpoint[0] ? endpoint : NULL, FALSE)) {
        widget_set_error (GTK_WIDGET (wg_peer->entry_endpoint));
        peer_is_valid = FALSE;
    }

    widget_unset_error (GTK_WIDGET (wg_peer->entry_public_key));
    if (!nm_wireguard_peer_set_public_key (nm_wg_peer, public_key && public_key[0] ? public_key : NULL, FALSE)) {
        widget_set_error (GTK_WIDGET (wg_peer->entry_public_key));
        peer_is_valid = FALSE;
    }

    widget_unset_error (GTK_WIDGET (wg_peer->entry_psk));
    if (!nm_wireguard_peer_set_preshared_key (nm_wg_peer, psk && psk[0] ? psk : NULL, FALSE)) {
        ...
        nma_utils_update_password_storage (GTK_WIDGET (wg_peer->entry_psk),
                                           nm_wireguard_peer_get_preshared_key_flags (wg_peer->nm_wg_peer), NULL, NULL);
    }
    ...
    if (g_strcmp0 (peer_allowed_ips, allowed_ips) != 0) {
        ...
        for (guint i = 0; strv && strv[i]; i++) {
            if (!nm_wireguard_peer_append_allowed_ip (nm_wg_peer, g_strstrip (strv[i]), FALSE)) {
...
finish_setup (CEPageWireguard *self, gpointer unused, GError *error, gpointer user_data)
{
    ...
    self->setting_wireguard =
        (NMSettingWireGuard *) nm_connection_get_setting (self->connection, NM_TYPE_SETTING_WIREGUARD);
    ...
    gtk_widget_set_visible (self->empty_listbox, nm_setting_wireguard_get_peers_len (self->setting_wireguard) < 1);

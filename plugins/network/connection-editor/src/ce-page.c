...
{
    signals[CHANGED] = g_signal_new ("changed", G_TYPE_FROM_INTERFACE (iface), G_SIGNAL_RUN_FIRST, 0, NULL, NULL,
                                     g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

    signals[INITIALIZED] = g_signal_new ("initialized", G_TYPE_FROM_INTERFACE (iface), G_SIGNAL_RUN_FIRST, 0, NULL,
...
void
ce_page_complete_init (CEPage *self, NMConnection *connection, const gchar *setting_name, GVariant *secrets,
                       GError *error)
{
    ...
    if (!nm_connection_update_secrets (connection, setting_name, secrets, &update_error))
...
gchar **
ce_page_get_mac_list (NMClient *client, GType device_type, const gchar *mac_property)
{
...
void
ce_page_setup_mac_combo (GtkComboBoxText *combo, const gchar *current_mac, gchar **mac_list)
{
    ...
    for (m = mac_list; m && *m; m++) {
        ...
        if (current_mac && g_ascii_strncasecmp (*m, current_mac, current_mac_len) == 0
...
ce_page_setup_cloned_mac_combo (GtkComboBoxText *combo, const char *current, gboolean is_wifi)
{
    ...
    static const char *entries_wired[][2] = { { "preserve", N_("Preserve") },
                                                { "permanent", N_("Permanent") },
                                                  { "random", N_("Random") }, { "stable", N_("Stable") } };

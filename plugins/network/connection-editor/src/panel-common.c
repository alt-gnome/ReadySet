...
gchar *
panel_device_status_to_localized_string (NMDevice *nm_device, const gchar *speed)
{
    NMDeviceState state;
    GString *string;
    ...
    } else if (state == NM_DEVICE_STATE_DISCONNECTED && NM_IS_DEVICE_ETHERNET (nm_device)) {
        /* TRANSLATORS: device status */
    ...
    } else if (state == NM_DEVICE_STATE_UNAVAILABLE || state == NM_DEVICE_STATE_DISCONNECTED
               || state == NM_DEVICE_STATE_DEACTIVATING || state == NM_DEVICE_STATE_FAILED) {

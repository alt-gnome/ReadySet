...
eap_method_ca_cert_ignore_load (NMConnection *connection)
{
    ...
    g_object_set_data (G_OBJECT (s_8021x), IGNORE_CA_CERT_TAG, GUINT_TO_POINTER (ignore));
    g_object_set_data (G_OBJECT (s_8021x), IGNORE_PHASE2_CA_CERT_TAG, GUINT_TO_POINTER (phase2_ignore));
}
...
static void
update_complete (NetConnectionEditor *self, gboolean success)
{
...
static void
device_reapply_cb (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
...
static void
updated_connection_cb (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
    NetConnectionEditor *self = user_data;
    g_autoptr(GError) error = NULL;

    if (!nm_remote_connection_commit_changes_finish (NM_REMOTE_CONNECTION (source_object), res, &error)) {
    ...
    nm_device_reapply_async (self->device, NM_CONNECTION (self->orig_connection), 0, 0, NULL, device_reapply_cb,
                             self /* owned */);
}

static void
added_connection_cb (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
    NetConnectionEditor *self = user_data;
    g_autoptr(GError) error = NULL;
...
apply_clicked_cb (NetConnectionEditor *self)
{
    ...
    if (self->is_new_connection) {
        nm_client_add_connection_async (self->client, self->orig_connection, FALSE, NULL, added_connection_cb,
                                        g_object_ref (self));
    } else {
        nm_remote_connection_commit_changes_async (NM_REMOTE_CONNECTION (self->orig_connection), FALSE, NULL,
                                                   updated_connection_cb, g_object_ref (self));
    }
...
net_connection_editor_class_init (NetConnectionEditorClass *class)
{
    ...
    signals[DONE] = g_signal_new ("done", G_OBJECT_CLASS_TYPE (object_class), G_SIGNAL_RUN_FIRST, 0, NULL, NULL, NULL,
...
static void
nm_connection_editor_watch_cb (GPid pid, gint status, gpointer user_data)
{
...
net_connection_editor_do_fallback (NetConnectionEditor *self, const gchar *type)
{
    ...
    g_spawn_async_with_pipes (NULL, argv, NULL, G_SPAWN_DO_NOT_REAP_CHILD | G_SPAWN_SEARCH_PATH, NULL, NULL, &child_pid,
...
static void
get_secrets_cb (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
...
static void
get_secrets_for_page (NetConnectionEditor *self, CEPage *page, const gchar *setting_name)
{
    GetSecretsInfo *info;
    ...
    nm_remote_connection_get_secrets_async (NM_REMOTE_CONNECTION (self->orig_connection), setting_name,
                                            self->cancellable, get_secrets_cb, info);
}
...
static void
net_connection_editor_set_connection (NetConnectionEditor *self, NMConnection *connection)
{
    GSList *pages, *l;
    NMSettingConnection *sc;
    const gchar *type;
    ...
    self->is_new_connection = !nm_client_get_connection_by_uuid (self->client, nm_connection_get_uuid (connection));
    ...
    add_page (self,
              CE_PAGE (ce_page_details_new (self->connection, self->device, self->ap, self, self->is_new_connection)));
...
static NMConnection *
complete_vpn_connection (NetConnectionEditor *self, NMConnection *connection, GType setting_type)
{
    ...
    if (!nm_setting_connection_get_connection_type (s_con)) {
        g_object_set (s_con, NM_SETTING_CONNECTION_TYPE, nm_setting_get_name (s_type), NULL);
    }
...
select_vpn_type (NetConnectionEditor *self, GtkListBox *list)
{
    ...
    for (iter = vpn_plugins; iter; iter = iter->next) {
        ...
        g_object_get (plugin, NM_VPN_EDITOR_PLUGIN_NAME, &name, NM_VPN_EDITOR_PLUGIN_DESCRIPTION, &desc,
                      NM_VPN_EDITOR_PLUGIN_SERVICE, &service_name, NULL);
    ...
    g_autofree gchar *desc_markup = g_markup_printf_escaped ("<span size='smaller'>%s</span>", desc);
...
static void
permission_changed (NetConnectionEditor *self, NMClientPermission permission, NMClientPermissionResult result)
{
...
NetConnectionEditor *
net_connection_editor_new (NMConnection *connection, NMDevice *device, NMAccessPoint *ap, NMClient *client)
{
    ...
    g_signal_connect_object (self->client, "permission-changed", G_CALLBACK (permission_changed), self,
                             G_CONNECT_SWAPPED);
...
static void
forgotten_cb (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
    ...
    if (!nm_remote_connection_delete_finish (connection, res, &error)) {
        if (!g_error_matches (error, G_IO_ERROR, G_IO_ERROR_CANCELLED))
            g_warning ("Failed to delete connection %s: %s", nm_connection_get_id (NM_CONNECTION (connection)),
                       error->message);
        return;
    }
...
{
    nm_remote_connection_delete_async (NM_REMOTE_CONNECTION (self->orig_connection), self->cancellable, forgotten_cb,
                                       self);
}

void
net_connection_editor_set_title (NetConnectionEditor *self, const gchar *title)
{

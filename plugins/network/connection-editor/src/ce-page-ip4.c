...
G_DEFINE_FINAL_TYPE_WITH_CODE (CEPageIP4, ce_page_ip4, ADW_TYPE_BIN,
...
update_row_gateway_sensitivity (CEPageIP4 *self)
{
    ...
    for (child = gtk_widget_get_first_child (self->address_list); child; child = gtk_widget_get_next_sibling (child)) {
        GtkWidget *entry;
...
static void
add_address_row (CEPageIP4 *self, const gchar *address, const gchar *network, const gchar *gateway)
{
    GtkWidget *row;
    GtkWidget *row_box;
    GtkWidget *widget;
    ...
    gtk_widget_set_hexpand (widget, TRUE);
    gtk_accessible_update_relation (GTK_ACCESSIBLE (widget), GTK_ACCESSIBLE_RELATION_LABELLED_BY,
                                    self->address_netmask_label, NULL, -1);
    gtk_box_append (GTK_BOX (row_box), widget);
...
add_address_box (CEPageIP4 *self)
{
    ...
    for (i = 0; i < nm_setting_ip_config_get_num_addresses (self->setting); i++) {
        ...
        add_address_row (self, nm_ip_address_get_address (addr), network,
...
static void
add_route_row (CEPageIP4 *self, const gchar *address, const gchar *netmask, const gchar *gateway, gint metric)
{
    GtkWidget *row;
    GtkWidget *row_box;
    GtkWidget *widget;
    ...
    gtk_widget_set_hexpand (widget, TRUE);
    gtk_accessible_update_relation (GTK_ACCESSIBLE (widget), GTK_ACCESSIBLE_RELATION_LABELLED_BY,
                                    self->routes_netmask_label, NULL, -1);
    gtk_box_append (GTK_BOX (row_box), widget);
    ...
    gtk_widget_set_hexpand (widget, TRUE);
    gtk_accessible_update_relation (GTK_ACCESSIBLE (widget), GTK_ACCESSIBLE_RELATION_LABELLED_BY,
                                    self->routes_metric_label, NULL, -1);
    gtk_box_append (GTK_BOX (row_box), widget);
...
add_route_config_box (CEPageIP4 *self)
{
    ...
    for (i = 0; i < nm_setting_ip_config_get_num_routes (self->setting); i++) {
        ...
        add_route_row (self, nm_ip_route_get_dest (route), netmask, nm_ip_route_get_next_hop (route),
...
ui_to_setting (CEPageIP4 *self)
{
    ...
    for (child = gtk_widget_get_first_child (self->address_list); add_addresses && child;
        ...
        if (ce_ip_address_entry_is_empty (address_entry) && ce_netmask_entry_is_empty (netmask_entry)
    ...
    for (child = gtk_widget_get_first_child (self->routes_list); add_routes && child;
        ...
        route = nm_ip_route_new (AF_INET, gtk_editable_get_text (GTK_EDITABLE (address_entry)),
    ...
    g_object_set (self->setting, NM_SETTING_IP_CONFIG_METHOD, method, NM_SETTING_IP_CONFIG_ADDRESSES, addresses,
                  NM_SETTING_IP_CONFIG_GATEWAY, default_gateway, NM_SETTING_IP_CONFIG_DNS,
                  dns_servers ? dns_servers->pdata : NULL, NM_SETTING_IP_CONFIG_ROUTES, routes,
                  NM_SETTING_IP_CONFIG_IGNORE_AUTO_DNS, ignore_auto_dns, NM_SETTING_IP_CONFIG_IGNORE_AUTO_ROUTES,
                  ignore_auto_routes, NM_SETTING_IP_CONFIG_NEVER_DEFAULT, never_default, NULL);
...
static void
on_ip4_method_activated_cb (GSimpleAction *action, GVariant *parameter, gpointer user_data)
{
...
static gboolean
ce_page_ip4_validate (CEPage *self, NMConnection *connection, GError **error)
{
...
CEPageIP4 *
ce_page_ip4_new (NMConnection *connection, NMClient *client)
{
    CEPageIP4 *self;

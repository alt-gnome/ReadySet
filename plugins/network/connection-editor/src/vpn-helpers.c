    ...
    while (p) {
        ...
        if (nm_vpn_plugin_info_load_editor_plugin (plugin_info, &error))
            ...
            if (!nm_vpn_plugin_info_get_plugin (plugin_info)
                ...
                g_message ("vpn: (%s,%s) cannot load legacy-only plugin", nm_vpn_plugin_info_get_name (plugin_info),
                ...
                g_message ("vpn: (%s,%s) file \"%s\" not found. Did you install the client package?",
                           nm_vpn_plugin_info_get_name (plugin_info), nm_vpn_plugin_info_get_filename (plugin_info),
                           nm_vpn_plugin_info_get_plugin (plugin_info));
            } else {
                g_warning ("vpn: (%s,%s) could not load plugin: %s", nm_vpn_plugin_info_get_name (plugin_info),
                           nm_vpn_plugin_info_get_filename (plugin_info), error->message);
            }
        }
...
import_vpn_from_file_cb (GObject *source_object, GAsyncResult *result, gpointer user_data)
{
    ...
    if (!connection) {
        ...
        err_dialog = adw_alert_dialog_new (_("Cannot Import VPN Connection"), NULL);
        ...
        adw_alert_dialog_add_response (ADW_ALERT_DIALOG (err_dialog), "ok", _("_OK"));

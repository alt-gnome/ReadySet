...
sync_dns_entry_warning (CEPageIP6 *self)
{
    ...
    if (gtk_entry_get_text_length (self->dns_entry) && gtk_switch_get_active (self->auto_dns_switch)
        ...
        gtk_entry_set_icon_tooltip_text (self->dns_entry, GTK_ENTRY_ICON_SECONDARY,
                                         _("Automatic DNS is enabled. Did you intend to disable Automatic DNS?"));
...
method_changed (CEPageIP6 *self)
{
    ...
    if (g_str_equal (method, "disabled") || g_str_equal (method, "shared")) {
        addr_enabled = FALSE;
        dns_enabled = FALSE;
    ...
    gtk_widget_set_visible (GTK_WIDGET (self->auto_dns_label), auto_dns_enabled);
    gtk_widget_set_visible (GTK_WIDGET (self->auto_dns_switch), auto_dns_enabled);
    gtk_widget_set_visible (GTK_WIDGET (self->auto_routes_label), auto_routes_enabled);
    gtk_widget_set_visible (GTK_WIDGET (self->auto_routes_switch), auto_routes_enabled);
...
update_row_sensitivity (CEPageIP6 *self, GtkWidget *list)
{
    ...
    for (child = gtk_widget_get_first_child (GTK_WIDGET (list)); child; child = gtk_widget_get_next_sibling (child)) {
        GtkWidget *button;
...
static void
add_address_row (CEPageIP6 *self, const gchar *address, const gchar *network, const gchar *gateway)
{
    GtkWidget *row;
    GtkWidget *row_box;
    GtkWidget *widget;
    ...
    gtk_accessible_update_relation (GTK_ACCESSIBLE (widget), GTK_ACCESSIBLE_RELATION_LABELLED_BY,
                                    self->address_address_label, NULL, -1);
    ...
    gtk_widget_set_hexpand (widget, TRUE);
    gtk_accessible_update_relation (GTK_ACCESSIBLE (widget), GTK_ACCESSIBLE_RELATION_LABELLED_BY,
                                    self->address_prefix_label, NULL, -1);
    gtk_box_append (GTK_BOX (row_box), widget);
    ...
    gtk_widget_set_hexpand (widget, TRUE);
    gtk_accessible_update_relation (GTK_ACCESSIBLE (widget), GTK_ACCESSIBLE_RELATION_LABELLED_BY,
                                    self->address_gateway_label, NULL, -1);
    gtk_box_append (GTK_BOX (row_box), widget);
    ...
    gtk_accessible_update_property (GTK_ACCESSIBLE (delete_button), GTK_ACCESSIBLE_PROPERTY_LABEL,
                                    _("Delete Address"), -1);
...
add_dns_section (CEPageIP6 *self)
{
    ...
    g_signal_connect_object (self->auto_dns_switch, "notify::active", G_CALLBACK (ce_page_changed), self,
                             G_CONNECT_SWAPPED);
    g_signal_connect_object (self->auto_dns_switch, "notify::active", G_CALLBACK (sync_dns_entry_warning), self,
                             G_CONNECT_SWAPPED);
    ...
    g_signal_connect_object (self->dns_entry, "notify::text", G_CALLBACK (sync_dns_entry_warning), self,
                             G_CONNECT_SWAPPED);
...
static void
add_dns_search_section (CEPageIP6 *self)
{
    GString *string;
    gint i;
    ...
    g_signal_connect_object (self->dns_search_entry, "notify::text", G_CALLBACK (ce_page_changed), self,
                             G_CONNECT_SWAPPED);
...
static void
add_route_row (CEPageIP6 *self, const gchar *address, const gchar *prefix, const gchar *gateway, const gchar *metric)
{
    GtkWidget *row;
    GtkWidget *row_box;
    GtkWidget *widget;
    ...
    gtk_widget_set_hexpand (widget, TRUE);
    gtk_accessible_update_relation (GTK_ACCESSIBLE (widget), GTK_ACCESSIBLE_RELATION_LABELLED_BY,
                                    self->routes_address_label, NULL, -1);
    gtk_box_append (GTK_BOX (row_box), widget);
    ...
    gtk_widget_set_hexpand (widget, TRUE);
    gtk_accessible_update_relation (GTK_ACCESSIBLE (widget), GTK_ACCESSIBLE_RELATION_LABELLED_BY,
                                    self->routes_prefix_label, NULL, -1);
    gtk_box_append (GTK_BOX (row_box), widget);
    ...
    gtk_widget_set_hexpand (widget, TRUE);
    gtk_accessible_update_relation (GTK_ACCESSIBLE (widget), GTK_ACCESSIBLE_RELATION_LABELLED_BY,
                                    self->routes_gateway_label, NULL, -1);
    gtk_box_append (GTK_BOX (row_box), widget);
...
add_route_config_box (CEPageIP6 *self)
{
    ...
    g_signal_connect_object (self->auto_routes_switch, "notify::active", G_CALLBACK (ce_page_changed), self,
                             G_CONNECT_SWAPPED);
    ...
    for (i = 0; i < nm_setting_ip_config_get_num_routes (self->setting); i++) {
        ...
        add_route_row (self, nm_ip_route_get_dest (route), prefix, nm_ip_route_get_next_hop (route), metric);
    }
...
ui_to_setting (CEPageIP6 *self)
{
    ...
    if (g_str_equal (method, NM_SETTING_IP6_CONFIG_METHOD_MANUAL)) {
        ...
        g_object_set (G_OBJECT (self->setting), NM_SETTING_IP_CONFIG_GATEWAY, NULL, NULL);
    }

    for (child = gtk_widget_get_first_child (self->address_list); add_addresses && child;
        ...
        if (!ce_ip_address_entry_is_empty (gateway_entry))
            g_object_set (G_OBJECT (self->setting), NM_SETTING_IP_CONFIG_GATEWAY,
                          gtk_editable_get_text (GTK_EDITABLE (gateway_entry)), NULL);
    ...
    for (child = gtk_widget_get_first_child (self->routes_list); add_routes && child;
         child = gtk_widget_get_next_sibling (child)) {
        GtkWidget *row = child;
        CEIPAddressEntry *address_entry;
        CEIPAddressEntry *gateway_entry;
        ...
        route = nm_ip_route_new (AF_INET6, gtk_editable_get_text (GTK_EDITABLE (address_entry)), prefix,
                                 gtk_editable_get_text (GTK_EDITABLE (gateway_entry)), metric, NULL);
    ...
    g_object_set (self->setting, NM_SETTING_IP_CONFIG_METHOD, method, NM_SETTING_IP_CONFIG_IGNORE_AUTO_DNS,
                  ignore_auto_dns, NM_SETTING_IP_CONFIG_IGNORE_AUTO_ROUTES, ignore_auto_routes,
...
static void
on_ip6_method_activated_cb (GSimpleAction *action, GVariant *parameter, gpointer user_data)
{
...
static gboolean
ce_page_ip6_validate (CEPage *self, NMConnection *connection, GError **error)
{
...
ce_page_ip6_class_init (CEPageIP6Class *klass)
{
    ...
    gtk_widget_class_bind_template_child (widget_class, CEPageIP6, dns_search_box);
    gtk_widget_class_bind_template_child (widget_class, CEPageIP6, dns_search_entry);
...
CEPageIP6 *
ce_page_ip6_new (NMConnection *connection, NMClient *client)
{
    CEPageIP6 *self;

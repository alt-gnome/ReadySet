...
connect_wifi_page (CEPageWifi *self)
{
    ...
    mac_list = ce_page_get_mac_list (self->client, NM_TYPE_DEVICE_WIFI, NM_DEVICE_WIFI_PERMANENT_HW_ADDRESS);
...
ui_to_setting (CEPageWifi *self)
{
    ...
    g_object_set (self->setting, NM_SETTING_WIRELESS_SSID, ssid, NM_SETTING_WIRELESS_BSSID, bssid,
                  NM_SETTING_WIRELESS_MAC_ADDRESS, device_mac, NM_SETTING_WIRELESS_CLONED_MAC_ADDRESS, cloned_mac,
                  NULL);
}
...
CEPageWifi *
ce_page_wifi_new (NMConnection *connection, NMClient *client)
{
    CEPageWifi *self;
